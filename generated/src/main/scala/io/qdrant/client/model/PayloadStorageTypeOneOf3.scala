...
case class PayloadStorageTypeOneOf3(
  @named("type") `type`: PayloadStorageTypeOneOf3Enums.Type
)

object PayloadStorageTypeOneOf3Enums:
  enum Type:
    case `in_ram_mmap`
      ...
      CodecMakerConfig
        .withAdtLeafClassNameMapper { x =>
          JsonCodecMaker.simpleClassName(x) match
            case "in_ram_mmap" => "in_ram_mmap"
        }
        .withDiscriminatorFieldName(scala.None)
    }
end PayloadStorageTypeOneOf3Enums
